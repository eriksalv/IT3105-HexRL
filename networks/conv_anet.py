...
class ConvActorNet(ActorNetwork):
    def __init__(self, k: int, anet_config_name: str, saved_weights_file=None, contains_bridges=False, padding=2):
        super().__init__(k, anet_config_name, saved_weights_file, contains_bridges, padding)
    ...
    def prepare_input_cases(self, inputs: list[np.ndarray]) -> np.ndarray:
        ...
        current_player_inputs = np.stack([np.full((self.k, self.k), case[-1]) for case in inputs])

        input_cases = np.zeros(shape=(len(inputs), *self.input_shape))
